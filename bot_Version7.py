...
async def after_subscribed(call: CallbackQuery):
...
async def handle_content_generation(call: CallbackQuery):
...
async def publish_or_ignore(call: CallbackQuery):
    uid = call.from_user.id
...
async def add_channel_step(message: Message):
    uid = message.from_user.id
...
async def ask_del_channel(call: CallbackQuery):
    uid = call.from_user.id
...
async def vip_status(call: CallbackQuery):
    uid = call.from_user.id
    if USERS[uid]["vip"]:
...
async def manager_notice(call: CallbackQuery):
    if GLOBAL_NOTIF:
        kb = InlineKeyboardMarkup().add(
...
async def ask_global_notif(call: CallbackQuery):
    await call.message.edit_text("أرسل نص الإشعار المراد نشره للجميع.")

@dp.message(lambda message: message.text and message.text.startswith("إشعار:") and message.from_user.id in ADMIN_IDS)
async def set_global_notif(message: Message):
    global GLOBAL_NOTIF
...
async def del_global(call: CallbackQuery):
    global GLOBAL_NOTIF
    GLOBAL_NOTIF = None
    await call.message.edit_text("تم حذف الإشعار العام.")

@dp.callback_query(lambda c: c.data == "manual_vip")
async def manual_vip(call: CallbackQuery):
    await call.message.edit_text("أرسل رقم المستخدم لتفعيل VIP يدويًا.")

@dp.message(lambda message: message.text and message.text.isdigit() and message.from_user.id in ADMIN_IDS)
async def activate_vip_manually(message: Message):
...
async def review_users(call: CallbackQuery):
    txt = ""
...
async def report_users(call: CallbackQuery):
    total = len(USERS)
...
async def manage_referrals(call: CallbackQuery):
    txt = ""
    for uid, data in USERS.items():
        if data["ref"]:
...
async def set_required_channel(call: CallbackQuery):
    await call.message.edit_text("أرسل اسم القناة الإلزامية الجديد (مثال: @new_channel).")

@dp.message(lambda message: message.text and message.text.startswith("@") and message.from_user.id in ADMIN_IDS)
async def change_required_channel(message: Message):
...
async def advanced_settings(call: CallbackQuery):
