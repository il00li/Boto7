...
def get_main_menu(is_admin=False):
    ...
    kb.add(
        InlineKeyboardButton("🧠 توليد محتوى", callback_data="generate_content"),
        ...
        InlineKeyboardButton("⭐ العضوية VIP", callback_data="vip_status"),
        InlineKeyboardButton("📢 إشعار المدير", callback_data="manager_notice"),
        InlineKeyboardButton("🔒 إدارة المغادرين", callback_data="manage_leavers"),
        InlineKeyboardButton("⚙️ إعداداتي", callback_data="my_settings")
    )
    if is_admin:
        kb.add(InlineKeyboardButton("👨‍💼 مدير النظام", callback_data="admin_panel"))
    return kb

def get_content_menu():
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(
        InlineKeyboardButton("عبارة سوداوية باللهجة المصرية", callback_data="content_sod_masri"),
        InlineKeyboardButton("عبارة تحفيزية قصيرة", callback_data="content_motiv_short"),
        InlineKeyboardButton("نكتة سوداوية لكن مضحكة", callback_data="content_dark_joke"),
        InlineKeyboardButton("جملة فلسفية عن الحياة", callback_data="content_philosophy"),
        InlineKeyboardButton("عبارة غامضة رمزية", callback_data="content_symbolic"),
        InlineKeyboardButton("اقتباس حزين من الأدب أو الشعر", callback_data="content_sad_quote"),
    )
    return kb
...
def get_admin_panel():
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton("📢 إرسال إشعار عام", callback_data="send_global"),
        InlineKeyboardButton("❌ حذف الإشعار العام", callback_data="del_global"),
        InlineKeyboardButton("📡 تعيين قناة الاشتراك الإجباري", callback_data="set_required_channel"),
        InlineKeyboardButton("👤 تفعيل عضوية VIP يدويًا", callback_data="manual_vip"),
        InlineKeyboardButton("🔍 مراجعة حالة المستخدمين", callback_data="review_users"),
        InlineKeyboardButton("🧑‍💼 إدارة روابط الإحالة والدعوات", callback_data="manage_referrals"),
        InlineKeyboardButton("📊 تقرير عام لحالة المستخدمين", callback_data="report_users"),
...
async def start_msg(message: types.Message):
    ...
    if not await check_subscription(uid):
        kb = InlineKeyboardMarkup().add(
            InlineKeyboardButton("✅ اشتركت", callback_data="subscribed"))
...
# --------- توليد المحتوى ---------
@dp.callback_query_handler(lambda c: c.data == "generate_content")
...
async def handle_content_generation(call: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton("✅ نشر المحتوى", callback_data=f"publish_{content_type}"),
        InlineKeyboardButton("❌ تجاهل المحتوى", callback_data="ignore_content")
    )
    await call.message.edit_text(f"المحتوى:\n\n{text}", reply_markup=kb)
    USERS[call.from_user.id]["last_content"] = text

@dp.callback_query_handler(lambda c: c.data.startswith("publish_") or c.data == "ignore_content")
...
# --------- جدولة النشر ---------
@dp.callback_query_handler(lambda c: c.data == "schedule_post")
async def schedule_menu(call: types.CallbackQuery):
    kb = InlineKeyboardMarkup().add(
        InlineKeyboardButton("كل 6 ساعات", callback_data="sched_6"),
        InlineKeyboardButton("كل 12 ساعة", callback_data="sched_12"),
        InlineKeyboardButton("كل 24 ساعة", callback_data="sched_24"),
    )
    await call.message.edit_text("اختر توقيت النشر التلقائي:", reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data.startswith("sched_"))
...
async def channel_menu(call: types.CallbackQuery):
    ...
    kb.add(
        ...
        InlineKeyboardButton("📑 عرض القنوات المرتبطة", callback_data="list_channels"),
    )
...
async def ask_del_channel(call: types.CallbackQuery):
    ...
    if USERS[uid]["channels"]:
        ...
        for ch in USERS[uid]["channels"]:
            kb.add(InlineKeyboardButton(f"حذف {ch}", callback_data=f"remove_{ch}"))
...
# --------- VIP ---------
@dp.callback_query_handler(lambda c: c.data == "vip_status")
...
# --------- إشعار المدير ---------
@dp.callback_query_handler(lambda c: c.data == "manager_notice")
async def manager_notice(call: types.CallbackQuery):
    if GLOBAL_NOTIF:
        kb = InlineKeyboardMarkup().add(
            InlineKeyboardButton("✔️ تم نشر الإشعار", callback_data="notif_done"),
            InlineKeyboardButton("❌ حذف الإشعار", callback_data="notif_del"),
        )
        await call.message.edit_text(f"إشعار المدير:\n{GLOBAL_NOTIF}", reply_markup=kb)
    else:
        await call.message.edit_text("لا يوجد إشعار حالياً.")

@dp.callback_query_handler(lambda c: c.data == "notif_done")
async def notif_done(call: types.CallbackQuery):
    await call.message.edit_text("تمت قراءة الإشعار.")

@dp.callback_query_handler(lambda c: c.data == "notif_del")
...
# --------- إدارة المغادرين ---------
@dp.callback_query_handler(lambda c: c.data == "manage_leavers")
...
# --------- إعداداتي ---------
@dp.callback_query_handler(lambda c: c.data == "my_settings")
...
@dp.callback_query_handler(lambda c: c.data == "send_global")
async def ask_global_notif(call: types.CallbackQuery):
    await call.message.edit_text("أرسل نص الإشعار المراد نشره للجميع.")

@dp.message_handler(lambda message: message.text.startswith("إشعار:") and message.from_user.id in ADMIN_IDS)
...
@dp.callback_query_handler(lambda c: c.data == "review_users")
...
@dp.callback_query_handler(lambda c: c.data == "report_users")
...
@dp.callback_query_handler(lambda c: c.data == "manage_referrals")
...
@dp.callback_query_handler(lambda c: c.data == "set_required_channel")
...
@dp.message_handler(lambda message: message.text.startswith("@") and message.from_user.id in ADMIN_IDS)
...
@dp.callback_query_handler(lambda c: c.data == "advanced_settings")
