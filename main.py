...
def process_search_term(message, user_id):
    ...
    if content_type == '3d':
        # استخدام Lummi للنماذج ثلاثية الأبعاد
        results = search_lummi(search_term)
    else:
        # استخدام Pixabay للأنواع الأخرى
...
def search_lummi(query):
    """البحث في Lummi للنماذج ثلاثية الأبعاد"""
    # Lummi API endpoint - هذا مثال، يجب استبداله بالنقطة الفعلية لـ Lummi API
    base_url = "https://api.lummi.ai/v1/search"
    ...
    try:
        logger.info(f"البحث في Lummi عن: {query} (3D Models)")
        response = requests.get(base_url, headers=headers, params=params, timeout=15)
        response.raise_for_status()
        data = response.json()
        
        # معالجة النتائج بناءً على هيكل استجابة Lummi
        # هذا مثال افتراضي، يجب تعديله حسب هيكل الاستجابة الفعلي لـ Lummi
        results = data.get('results', [])
        
        logger.info(f"تم العثور على {len(results)} نتيجة من Lummi")
        return results
    except Exception as e:
        logger.error(f"خطأ في واجهة Lummi: {e}")
        # في حالة فشل البحث في Lummi، نعود إلى البحث في Pixabay مع إضافة 3d للاستعلام
        logger.info("المحاولة في البحث في Pixabay كبديل...")
        return search_pixabay(query + ' 3d', 'photo')
...
def show_result(chat_id, user_id, message_id=None):
    ...
    caption += f"🌐 المصدر: {'Lummi' if content_type == '3d' else 'Pixabay'}\n"
    ...
    try:
        ...
        # إذا كانت النتيجة من Lummi (نموذج ثلاثي الأبعاد)
        elif content_type == '3d':
            # الحصول على رابط الصورة من النتيجة
            # هذا مثال افتراضي، يجب تعديله حسب هيكل استجابة Lummi الفعلي
            image_url = item.get('preview_url', item.get('image_url', ''))
            
            # إذا لم يكن هناك صورة معاينة، نستخدم أي صورة متاحة
            if not image_url and 'images' in item and len(item['images']) > 0:
                image_url = item['images'][0].get('url', '')
            
            # التحقق من صحة URL
            if not is_valid_url(image_url):
                # إذا لم يكن هناك صورة صالحة، نستخدم صورة افتراضية
                image_url = "https://via.placeholder.com/600x400/2c3e50/ffffff?text=3D+Model"
            ...
            if message_id:
                try:
                    ...
                    logger.error(f"فشل في تعديل رسالة النموذج ثلاثي الأبعاد: {e}")
...
def download_content(call):
    ...
    try:
        ...
        if content_type == 'video' and 'videos' in item:
            ...
            # للنماذج ثلاثية الأبعاد من Lummi
            # هذا مثال افتراضي، يجب تعديله حسب هيكل استجابة Lummi الفعلي
            model_url = item.get('download_url', item.get('url', ''))
            if model_url and is_valid_url(model_url):
                # إرسال رابط التنزيل مباشرة
                bot.send_message(UPLOAD_CHANNEL, f"📦 نموذج 3D للتحميل:\n{model_url}\n\n{caption}")
            else:
                # إذا لم يكن هناك رابط تنزيل، نرسل صورة المعاينة
                image_url = item.get('preview_url', item.get('image_url', ''))
                if image_url and is_valid_url(image_url):
                    bot.send_photo(UPLOAD_CHANNEL, image_url, caption=caption)
                else:
                    bot.send_message(UPLOAD_CHANNEL, caption)
        else:
            image_url = item.get('largeImageURL', item.get('webformatURL', ''))
            bot.send_photo(UPLOAD_CHANNEL, image_url, caption=caption)
    except Exception as e:
