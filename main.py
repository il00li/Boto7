...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📣 إشعار عام", callback_data="notifications")] if user_id == ADMIN_ID else None,
...
async def handle_content_generation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text(content, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_notification_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, user in users_db.items():
        try:
            await context.bot.send_message(uid, f"إشعار عام:\n{update.message.text}",
