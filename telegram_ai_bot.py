...
def vip_invite_check(user_id):
    ...
    # تحقق أن كل المدعوين مشتركين في القناة الإلزامية
    ...
    while True:
        ...
        for user_id, u in users.items():
            ...
            if auto.get("interval") and auto.get("next") and now >= auto["next"]:
                # توليد المحتوى ونشره في القنوات المرتبطة
...
def publish_content(call):
    ...
    if users[user_id]["invite_count"] == 5 and not is_vip(user_id):
        ...
        bot.send_message(user_id, "لقد وصلت إلى الحد الأقصى للنشر المجاني.\nادعُ 10 أشخاص واشترط اشتراكهم في القناة الإلزامية لتفعيل VIP.", reply_markup=kb)
...
def add_channel_step2(message):
    user_id = message.from_user.id
    ch = message.text
    # تحقق هل المستخدم مدير في القناة (يتطلب صلاحيات البوت في القناة!)
    try:
...
@bot.message_handler(func=lambda m: is_admin(m.from_user.id) and m.reply_to_message is None)
def admin_notif_text(message):
    notifications.append({"msg_id": message.message_id, "text": message.text})
    # إرسال لجميع المستخدمين وقنواتهم
    for uid in users:
        try:
